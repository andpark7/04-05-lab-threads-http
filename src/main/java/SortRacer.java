...
public class SortRacer extends Thread {
	...
	{
		/*SortRacer sort = new SortRacer();
		MergeRun merge = new sort.Merge();*/
		/** Merge Sort **/
		...
		Thread mergeThread = new Thread(new mergeRun());
		Thread quickThread = new Thread(new quickRun());
		mergeThread.start();
		quickThread.start();
	...
	public static class mergeRun implements Runnable {
		public void run() {
	...
	public static class quickRun implements Runnable {
		public void run() {
			SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss.SSSS"); //for output
			Integer[] nums;
			nums = shuffled((int)Math.pow(10,7), 498); //a list of shuffled 10 million numbers
